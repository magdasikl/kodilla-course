...
public class ShopService {
    ...
    public BigDecimal calculateValue(Long orderId) {
        Iterator<Order> orderIterator = orders.stream()
                ...
                .iterator();
        while (orderIterator.hasNext()) {
            Order theOrdeer = orderIterator.next();
            return theOrdeer.calculateValue();
        }
    ...
    public boolean doPayment(Long orderId) {
        ...
        while (orderIterator.hasNext()) {
            ...
            if (theOrder.isPaid()) {
                ...
                Random gerenrator = new Random();
                theOrder.setPaid(gerenrator.nextBoolean());
    ...
    public boolean verifyOrder(Long orderId) {
        ...
        while (orderIterator.hasNext()) {
            ...
            if (!theOrder.isVerified()) {
                theOrder.setVerified(result && gerenrator.nextBoolean());
            }
    ...
    public boolean submitOrder(Long orderId) {
        ...
        while (orderIterator.hasNext()) {
            Order theOrder = orderIterator.next();
            if (theOrder.isVerified()) {
                theOrder.setSumitted(true);
            }
            return theOrder.isSumitted();
        }
        return false;
    }
