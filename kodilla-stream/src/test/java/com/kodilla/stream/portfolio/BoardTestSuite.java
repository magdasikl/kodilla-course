...
//        When
        ...
        List<Task> taks = project.getTaskLists().stream()
                .filter(undoneTasks::contains)
                .flatMap(taskList -> taskList.getTasks().stream())
                .filter(task -> task.getDeadline().isBefore(LocalDate.now()))
                .collect(toList());

        Assert.assertEquals(1,taks.size());
        Assert.assertEquals("HQLs for analysis",taks.get(0).getTitle());
...
//      Then
        long quantityDayOfTask = project.getTaskLists().stream()
                .filter(inProgressTask::contains)
                .flatMap(taskList -> taskList.getTasks().stream())
                .map(task -> task.getCreated())
    ...
    long avarageTask = project.getTaskLists().stream()
            .filter(inProgressTask::contains)
            .flatMap(taskList -> taskList.getTasks().stream())
            .count();

    long avarageOfTasks = quantityDayOfTask/avarageTask;

    Assert.assertEquals(10,avarageOfTasks );
}
}
